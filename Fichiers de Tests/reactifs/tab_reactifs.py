...
class GestionReactifs(QWidget):
    ...
    def on_consumption_unit_changed(self, new_unit):
        """
        Gère le changement d'unité dans le comboBox de consommation (comboBox_unite_consommation_par_unite_de_temps).

        Selon l'unité sélectionnée, on ouvre une boîte de dialogue unifiée pour saisir
        les paramètres nécessaires et on met à jour les autres champs de l'interface de la façon suivante :

        - Cas des packagings (boîte, kit, sachet, coffret) :
        * Fixer comboBox_unite_consommation_par_unite_de_temps, comboBox_unite_qte_totale_conditionnement_firstRow
            et comboBox_unite_physique_firstRow sur "test".
        * lineEdit_qte_par_unite_de_test_firstRow = "1".
        * Ouvrir un QDialog pour saisir le nombre total de tests par packaging.

        - Cas des contenants (flacon, tube, bouteille) :
        * Ouvrir un QDialog pour saisir la quantité par test et la quantité totale (volume/masse).
        * Après validation, remplir lineEdit_qte_par_unite_de_test_firstRow, comboBox_unite_physique_firstRow,
            lineEdit_qte_totale_conditionnement_firstRow, comboBox_unite_qte_totale_conditionnement_firstRow
            et comboBox_unite_consommation_par_unite_de_temps.

        - Cas massique/volumique (ml, L, µl, mg, g, kg) :
        * Convertir directement, si nécessaire, la valeur affichée dans lineEdit_consommation_par_unite_de_temps_firstRow
            vers la nouvelle unité, en vérifiant la compatibilité des unités.
        """
        packaging_units = ["boîte", "kit", "sachet", "coffret"]
        container_units = ["flacon", "tube", "bouteille"]
        mass_volume_units = ["ml", "L", "µl", "mg", "g", "kg"]

        # Cas packaging
        if new_unit in packaging_units:
            self.comboBox_unite_consommation_par_unite_de_temps.setCurrentText("test")
            self.comboBox_unite_qte_totale_conditionnement_firstRow.setCurrentText("test")
            self.lineEdit_qte_par_unite_de_test_firstRow.setText("1")
            self.comboBox_unite_physique_firstRow.setCurrentText("test")
            # Ouvrir la boîte de dialogue pour saisir le nombre total de tests
            self.open_unified_dialog(new_unit)

        # Cas contenant
        elif new_unit in container_units:
            self.open_unified_dialog(new_unit)

        # Cas massique/volumique
        elif new_unit in mass_volume_units:
            ...
            try:
                ...
                value = float(value_part.split()[0])       # ex: "10.00"
                old_unit = value_part.split()[1]          # ex: "ml"
                # Vérifier la compatibilité et convertir si nécessaire
                if self.calculator.are_units_compatible(old_unit, new_unit):
                    ...
                    self.show_error_message(f"Conversion impossible entre {old_unit} et {new_unit}")
                    self.comboBox_unite_consommation_par_unite_de_temps.setCurrentText(old_unit)
