...
class GestionReactifs(QWidget):
    ...
    def on_consumption_unit_changed(self, new_unit):
        ...
        if new_unit in ["tube", "flacon"]:
            ...
            if dialog_result:
                ...
                self.comboBox_unite_qte_totale_conditionnement_firstRow.setCurrentText(unit)
                ...
                self.comboBox_unite_physique_firstRow.setCurrentText(unit)
                self.comboBox_unite_consommation_par_unite_de_temps.setCurrentText(unit)
                
                # Recalculer la consommation
                self.update_consumption()
        elif new_unit in ["boîte", "coffret", "sachet", "kit"]:
            # Ouvrir un dialogue pour saisir le nombre total de tests pour les conditionnements
            dialog_result = self.open_packaging_dialog(new_unit)
            if dialog_result:
                total_tests = dialog_result
                # Mettre à jour les champs correspondants pour les conditionnements
    ...
    def open_container_dialog(self, unit):
        ...
        Retourne un tuple (qty_per_test, total_qty, unit).
        """
        dialog = QDialog(self)
        ...
        qty_per_test_layout.addWidget(QLabel("Quantité par test :"))
        ...
        total_qty_layout.addWidget(QLabel(f"Volume ou masse totale du {unit} :"))
    ...
    def open_packaging_dialog(self, unit):
        ...
        layout.addWidget(QLabel(f"Nombre total de tests par {unit} :"))
    ...
    def update_consumption(self):
        ...
        try:
            ...
            if consumption_unit in ["tube", "flacon"]:
                ...
                if consumption_unit in UNITS_CAL_CONT_CONFIRM:
                    result = f"{consumption:.2f} {consumption_unit}/{time_value} {time_unit}"
                else:
                    # Conversion nécessaire pour des unités spécifiques
                    result = f"{consumption:.2f} {consumption_unit}/{time_value} {time_unit}"
