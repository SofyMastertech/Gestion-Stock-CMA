from PySide6.QtWidgets import (
    ...
    QDialogButtonBox, QDateEdit, QSpacerItem, QSizePolicy, QMenu
)
...
class AddEditTestDialog(QDialog):
    ...
    def create_form(self, layout, data):
        ...
        # Nom analyte
        self.add_form_row(form_layout, "Nom analyte :", QComboBox(), "nom_analyte", data, add_to_layout=True,
        ...
        debut_date_edit.setDate(QDate.currentDate())  # Date actuelle par défaut
        fin_date_edit = QDateEdit(calendarPopup=True)
        fin_date_edit.setDate(QDate.currentDate())  # Date actuelle par défaut
    ...
    def calculate_fields(self):
        ...
        try:
            ...
            # Si "Tests Réalisés" est 0, tout doit être mis à 0
    ...
    def fetch_analytes_from_database(self):
        """Récupère la liste des analytes depuis la base de données."""
        try:
    ...
    def validate(self):
        """Valide les données saisies avant de fermer le dialogue."""
        try:
...
class TabTests(QWidget):
    ...
    def setup_ui(self):
        ...
        # Ajouter le combo box ici après le bouton "Rétablir"
        ...
        search_layout.addWidget(self.cmb_analytes)  # Déplacer ici
        ...
        # Création du premier bouton "Ajouter"
        add_button = QPushButton("Ajouter")
        add_button.clicked.connect(self.add)
        btn_layout.addWidget(add_button)

        # Ajout du spacer après le premier bouton
    ...
    def populate_analytes_combo(self):
        """
        Récupère les noms d'analytes depuis la base de données et les ajoute à la combobox.
        """
        try:
            analytes = self.database.get_all_analytes()
    ...
    def update_analyte_stats(self):
        ...
        try:
            # Démarrer un thread pour charger les tests associés à l'analyte
            self.thread = DatabaseWorkerThread(
                query="""
                    SELECT estimated_tests, performed_tests, usage_factor, loss_percentage
                    FROM Tests
    ...
    def update_average_stats(self, success, results):
        ...
        """
        if not success:
            QMessageBox.critical(self, "Erreur", f"Impossible de charger les statistiques : {results}")
            self.clear_stat_fields()
            return
        ...
        try:
            total_estimated_tests = sum(int(test['estimated_tests']) for test in results) / len(results)
            total_performed_tests = sum(int(test['performed_tests']) for test in results) / len(results)
            avg_usage_factor = sum(float(test['usage_factor']) for test in results) / len(results)
            avg_loss_percentage = sum(float(test['loss_percentage']) for test in results) / len(results)

            # Mise à jour des champs de l'interface utilisateur
            self.txt_estimated_tests.setText(f"{total_estimated_tests:.2f}")
            self.txt_performed_tests.setText(f"{total_performed_tests:.2f}")
            self.txt_usage_factor.setText(f"{avg_usage_factor:.2f}")
            self.txt_loss_percentage.setText(f"{avg_loss_percentage:.2f}%")
        except Exception as e:
    ...
    def load_data_from_database(self):
        """
        Charge les données des tests depuis la base de données de manière asynchrone en utilisant un thread.
        """
        query_tests = """
        ...
        JOIN Analytes ON Tests.analyte_id = Analytes.id
        """
        self.thread = DatabaseWorkerThread(query=query_tests, db_path="reactifs_database.db", fetch=True)
        self.thread.finished.connect(self.on_load_data_finished)
        self.thread.start()

    def on_load_data_finished(self, success, tests):
        """
        Callback appelé lorsque le chargement des données est terminé.
        """
        if not success:
    ...
    def create_table(self):
        """
        Crée et configure la table pour afficher les données.
        """
        table = QTableWidget()
        headers = [
    ...
    def add(self):
        """
        Ajoute un nouveau test via le dialogue.
        """
    ...
    def show_context_menu(self, position):
        """
        Affiche un menu contextuel stylisé avec des actions pour modifier ou supprimer une ligne.
        """
        menu = QMenu(self.table)

        # Styles CSS pour le menu
        menu.setStyleSheet("""
            QMenu {
            ...
            QMenu::item {
                padding: 6px 24px; /* Espacement entre les items */
                background-color: transparent;
            }

            QMenu::item:selected {
                background-color: #0078d7; /* Bleu foncé sur sélection */
                color: white; /* Texte blanc sur sélection */
            }

            QMenu::separator {
                height: 1px;
                background: #d3d3d3; /* Séparateur gris */
                margin: 4px 0; /* Espacement autour du séparateur */
            }
        """)

        # Actions du menu
        edit_action = QAction("Modifier", menu)
        edit_action.setIcon(QIcon(":/icons/edit.png"))  # Icône pour Modifier
        edit_action.triggered.connect(lambda: self.edit(self.table.currentRow()))

        delete_action = QAction("Supprimer", menu)
        delete_action.setIcon(QIcon(":/icons/delete.png"))  # Icône pour Supprimer
    ...
    def edit(self, row=None):
        """
        Modifie un test existant via le dialogue.
        """
        if row is None:
            row = self.table.currentRow()
        if row == -1:
    ...
    def update_test_in_database(self, test_id, data):
        """
        Met à jour un test dans la base de données.
        """
        try:
            analyte_id = self.database.get_analyte_id(data['nom_analyte'])
            if not analyte_id:
    ...
    def update_table_row(self, test_id, data):
        """
        Met à jour une ligne dans la table avec les nouvelles données.
        """
    ...
    def delete(self, row=None):
        """
        Supprime un test de la base de données et de l'interface utilisateur.
        """
        if row is None:
            row = self.table.currentRow()
        if row == -1:
    ...
    def on_save_all_finished(self, success, result):
        """
        Gère la fin de l'enregistrement de toutes les lignes.
        """
        if success:
    ...
    def export_pdf(self):
        """
        Exporte les données de la table vers un fichier PDF.
        """
        headers = [self.table.horizontalHeaderItem(i).text() for i in range(self.table.columnCount())]
        data = []
        for row in range(self.table.rowCount()):
        ...
        export_to_pdf(headers, data)

    def export_excel(self):
        """
        Exporte les données de la table vers un fichier Excel.
        """
        headers = [self.table.horizontalHeaderItem(i).text() for i in range(self.table.columnCount())]
        data = []
        for row in range(self.table.rowCount()):
    ...
    def show_explanation(self):
        """
        Affiche une explication des colonnes dans une boîte de dialogue.
        """
        explanation = (
            "Calculs des tests :\n"
    ...
    def get_row_data(self, row):
        """
        Récupère les données d'une ligne donnée.
        """
        data = {}
        headers = [self.table.horizontalHeaderItem(i).text() for i in range(self.table.columnCount())]
        for col, header in enumerate(headers):
    ...
    def on_row_selected(self, current, previous):
        """
        Met à jour les champs d'analyse lorsque vous sélectionnez une ligne.
        """
        row = current.row()
        if row >= 0:
            try:
    ...
    def dynamic_search(self):
        """
        Filtre dynamiquement les données dans la table selon le texte saisi.
        """
