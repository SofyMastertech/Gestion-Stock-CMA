...
class GestionReactifs(QWidget):
    ...
    def add_first_row_widgets(self, groupbox: QGroupBox):
        ...
        Organise les widgets en trois lignes et utilise une infobulle pour le champ Qte Totale/Conditionnement.
        """
        ...
        self.lineEdit_qte_totale_conditionnement_firstRow.setToolTip("Entrez la quantité totale par conditionnement")  # Infobulle au survol
        ...
        self.lineEdit_consommation_par_unite_de_temps_firstRow.setReadOnly(True)  # Lecture seule
    ...
    def on_consumption_unit_changed(self, new_unit):
        ...
        # Unités massiques ou volumiques
        mass_volume_units = ["ml", "L", "µl", "mg", "g", "kg"]

        if new_unit in packaging_units:
            # Changer l'unité physique en "test"
            self.comboBox_unite_physique_firstRow.setCurrentText("test")
            # Afficher un QDialog unifié pour saisir le nombre total de tests par packaging
            self.open_unified_dialog(new_unit)
        elif new_unit in container_units:
            # Afficher un QDialog unifié pour saisir la quantité par test et le volume total
            self.open_unified_dialog(new_unit)
        # Si l'unité est massique ou volumique
            ...
            try:
                ...
                old_unit = value_part.split()[1]
                # Récupérer l'unité physique actuelle
                physical_unit = self.comboBox_unite_physique_firstRow.currentText()
                ...
                if self.calculator.are_units_compatible(old_unit, new_unit):
                    ...
                    # Si les unités ne sont pas compatibles, afficher un message d'erreur
                    self.show_error_message(f"Conversion impossible entre {old_unit} et {new_unit}")
                    # Revenir à l'ancienne unité
                    self.comboBox_unite_consommation_par_unite_de_temps.setCurrentText(old_unit)
            except (ValueError, IndexError) as e:
                # En cas d'erreur, afficher un message et conserver l'ancienne unité
        ...
        try:
            if unit in ["flacon", "tube", "bouteille"]:
                ...
                if qty_per_test_unit != total_qty_unit:
                    if not self.are_units_compatible(qty_per_test_unit, total_qty_unit):
    ...
    def calculate_all(self):
        ...
        try:
            ...
            if consommation_unit != target_unit:
                if self.are_units_compatible(consommation_unit, target_unit):
            ...
            if calibration_unit != target_unit:
                if self.are_units_compatible(calibration_unit, target_unit):
            ...
            if controle_unit != target_unit:
                if self.are_units_compatible(controle_unit, target_unit):
            ...
            if dilution_unit != target_unit:
                if self.are_units_compatible(dilution_unit, target_unit):
